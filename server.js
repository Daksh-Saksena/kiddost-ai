...
- If the user's CURRENT MESSAGE mentions an area/locality within Bangalore (e.g. Koramangala, BTM, Whitefield, HSR Layout, Indiranagar, JP Nagar, Marathahalli, Electronic City, Jayanagar, etc.), say EXACTLY:
...
Consider the FULL conversation history carefully — do not confuse one child's age with another's.`
    ...
    try {
      ...
      if (!evErr && pastEvents && pastEvents.length > 0) {
        ...
        sessionStatusBlock = `\n\nSESSION HISTORY for this customer:\n- No previous sessions found.\n- This is a FIRST-TIME customer. When mentioning the first session price, use this exact line: "We suggest scheduling a one-hour introductory session at your convenience. For the first experience of our service, we are happy to offer it at a discounted price of ₹500 per hour." IMPORTANT: ONLY pitch the introductory session ONCE and never again. Treat the booking flow the same as a regular session after that.`;
      }
    } catch (e) {
        ...
        (async () => {
          try {
            ...
            if (messageBuffers[fullPhone] && messageBuffers[fullPhone].length > 0) {
              ...
              if (messageTimers[fullPhone]) clearTimeout(messageTimers[fullPhone]);
              messageTimers[fullPhone] = setTimeout(triggerAiProcessing, 300);
            }
          }
        })();
      };
